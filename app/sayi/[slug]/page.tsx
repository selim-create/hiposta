...
export default async function IssuePage({ params }: Props) {
  ...
  const issueSchema = {
    ...
    isPartOf: { "@type": "Periodical", name: newsletter.name, url: absoluteUrl(`/bultenler/${newsletter.slug}`) },
    inLanguage: "tr-TR",
  };
  const breadcrumbSchema = {
    "@context": "https://schema.org", "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Bültenler", item: absoluteUrl("/bultenler") },
      { "@type": "ListItem", position: 2, name: newsletter.name, item: absoluteUrl(`/bultenler/${newsletter.slug}`) },
    ...
    <article className="issue-page" style={style}>
      ...
      <header className="issue-hero">
        <div className="page-shell issue-hero__inner">
          <div className="breadcrumb"><Link href="/bultenler">Bültenler</Link><span>/</span><Link href={`/bultenler/${newsletter.slug}`}>{newsletter.name}</Link><span>/</span><span>{issue.displayDate}</span></div>
          <div className="issue-hero__grid">
            <div><PublicationMark publication={publication} linked={false} /><p className="eyebrow">{newsletter.name} · Web arşivi</p><h1>{issue.title}</h1>{issue.preheader && <p className="issue-hero__dek">{issue.preheader}</p>}</div>
            <aside><div className="issue-hero__date"><CalendarDays size={18} /><span><small>Yayın tarihi</small>{issue.displayDate}</span></div><Link className="inline-arrow-link" href={`/bultenler/${newsletter.slug}`}><ArrowLeft size={15} /> Tüm sayılar</Link></aside>
          </div>
        </div>
      </header>
              ...
              {items.map((item, index) => (
                <article className="issue-story" key={item.slug}>
                  ...
                  <div className="issue-story__content">
                    <div className="issue-story__meta"><span>{item.categoryShortName || item.categoryName || "Gündem"}</span>{item.premium && <span className="premium-pill"><LockKeyhole size={10} /> Premium</span>}</div>
