...
export default async function IssuePage({ params }: Props) {
  ...
  const issueSchema = { "@context": "https://schema.org", "@type": "Article", headline: issue.title, description: issue.preheader || `${issue.newsletterName} arşiv sayısı`, datePublished: issue.publishedAt, dateModified: issue.updatedAt || issue.publishedAt, publisher: { "@type": "Organization", name: "Hiposta", url: absoluteUrl("/"), logo: { "@type": "ImageObject", url: absoluteUrl("/brand/hiposta-wordmark-dark.png") } }, mainEntityOfPage: { "@type": "WebPage", "@id": canonical }, isPartOf: { "@type": "Periodical", name: newsletter.name, url: absoluteUrl(`/bultenler/${newsletter.slug}`) }, inLanguage: "tr-TR" };
