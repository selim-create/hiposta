  ...
  return (
    <div className="home-v2">
      <section className="home-manifesto page-shell">
        <div>
          <p className="eyebrow">Hiposta · Hip Medya yayın ağı</p>
          <h1>Okumaya değer olanı bul.<br /><span>İstediğini gelen kutuna al.</span></h1>
        </div>
        <div className="home-manifesto__copy">
          <p>Farklı yayınların editoryal sesini tek platformda keşfet. İçeriği oku, bültenini seç, premium dosyalara tek üyelikten ulaş.</p>
          <dl><div><dt>{catalog.stats.publications}</dt><dd>yayın</dd></div><div><dt>{catalog.stats.activeNewsletters}</dt><dd>aktif bülten</dd></div><div><dt>{catalog.stats.categories}</dt><dd>kategori</dd></div></dl>
        </div>
      </section>
      ...
      {lead && leadPublication && <section className="home-lead home-lead--v2 page-shell">
        <article className="home-lead__story">
          ...
          <h2><Link href={`/icerik/${lead.slug}`}>{lead.title}</Link></h2>
        ...
        <aside className="home-lead__aside">
          {dailyNewsletter && <div className="daily-newsletter daily-newsletter--v2">
            <div className="daily-newsletter__top"><span>Başlangıç bülteni</span><b>Ücretsiz</b></div>
            <div className="daily-newsletter__issue">HIPOSTA / GÜNDEM</div>
          ...
          <div className="signal-card signal-card--v2">
            <div className="signal-card__head"><span>Keşfet</span><span>Konular</span></div>
            {catalog.categories.slice(0, 3).map((category, index) => <Link key={category.slug} href={`/kategori/${category.slug}`}><span>0{index + 1}</span><p>{category.shortName}</p><ArrowUpRight size={14} /></Link>)}
          </div>
        </aside>
      </section>}
      ...
      {latest.length > 0 && <section className="section page-shell">
        <div className="section-heading section-heading--rule"><div><p className="eyebrow">Editör masası</p><h2>Bugün okumaya değer</h2></div><Link href="/arama">Tüm içerikler <ArrowUpRight size={15} /></Link></div>
        <div className="article-grid article-grid--three">{latest.map((article) => <ArticleCard key={article.slug} article={article} />)}</div>
      </section>}

      <section className="premium-band premium-band--v2">
        <div className="page-shell premium-band__inner">
          <div className="premium-band__seal">H<span>+</span></div>
          <div><p className="eyebrow">Hiposta Premium</p><h2>İlk paragrafta<br />kalma.</h2></div>
          <div className="premium-band__copy"><p>{catalog.stats.publications} yayının derin analizleri, özel dosyaları ve reklamsız okuma deneyimi tek üyelikte.</p><ul><li><Sparkles size={14} /> Tüm premium içerikler</li><li><Sparkles size={14} /> Hiposta Dergi</li><li><Sparkles size={14} /> Tek profil, tüm yayınlar</li></ul><Link className="button button--yellow" href="/premium">Premium’u keşfet <ArrowRight size={16} /></Link></div>
        </div>
      </section>

      <section className="section page-shell">
        <div className="section-heading section-heading--rule"><div><p className="eyebrow">Hip Medya ekosistemi</p><h2>Her yayının kendi sesi var.</h2></div><Link href="/yayinlar">{catalog.stats.publications} yayını gör <ArrowUpRight size={15} /></Link></div>
      ...
      <section className="newsletter-showcase newsletter-showcase--v2 page-shell">
        <div className="newsletter-showcase__heading"><div><p className="eyebrow">Gelen kutunu yeniden kur</p><h2>Bir bülten değil,<br />kişisel yayın akışın.</h2></div><p>Ekonomi sabah gelsin, tarifler iş çıkışından önce, haftanın en iyi fikirleri pazar günü. Seç, birleştir, tek yerden yönet.</p></div>
