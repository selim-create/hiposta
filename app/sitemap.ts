...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const fixed: MetadataRoute.Sitemap = [
    ...
    { url: absoluteUrl("/premium"), changeFrequency: "monthly", priority: 0.65 },
  ...
  if (catalog.source !== "core") return fixed;
  ...
  const activeCategorySlugs = new Set([
    ...
    ...articles.map((item) => item.categorySlug),
  ]);
  ...
  return [
    ...
    ...articles.map((item) => ({ url: absoluteUrl(`/icerik/${item.slug}`), lastModified: validDate(item.updatedAt) ?? validDate(item.publishedAt), changeFrequency: "weekly" as const, priority: 0.85 })),
