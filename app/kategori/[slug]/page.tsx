...
export default async function CategoryPage({ params }: Props) {
  ...
  const articleSlug = slug === "saglik-iyi-yasam" ? "iyi-yasam" : slug;
  const categoryArticles = getCategoryArticles(articleSlug);
      ...
      <section className="category-network page-shell">
        <div>
          ...
          {categoryPublications.map((publication) => (
            <div key={publication.slug} style={{ opacity: publication.isComingSoon ? 0.55 : 1 }}>
              <PublicationMark publication={publication} />
              <span>{publication.description}{publication.isComingSoon ? " · Yakında" : ""}</span>
            </div>
          ))}
        </div>
        <div>
          <p className="eyebrow">Doğrudan gelen kutuna</p><h2>Bültenler</h2>
          {categoryNewsletters.map((newsletter) => (
            <NewsletterCard key={newsletter.slug} newsletter={newsletter} publication={catalog.publications.find((item) => item.slug === newsletter.publicationSlug)} />
          ))}
