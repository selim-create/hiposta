...
export default async function SearchPage({ searchParams }: Props) {
  ...
  return (
    <section className="search-page page-shell">
      ...
      {newsletters.length > 0 && <section className="search-result-section"><div className="section-heading section-heading--small section-heading--rule"><div><p className="eyebrow">{newsletters.length} sonuç</p><h2>Bültenler</h2></div></div><div className="newsletter-grid">{newsletters.map((newsletter) => <NewsletterCard key={newsletter.slug} newsletter={newsletter} publication={catalog.publications.find((item) => item.slug === newsletter.publicationSlug)} showSubscriptionAction authenticated={Boolean(session)} verified={Boolean(session?.account.email_verified)} subscribed={activeSubscriptions.has(newsletter.slug)} source="search_newsletter_card" />)}</div></section>}
    </section>
  );
}
