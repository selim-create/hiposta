...
type ApiIssue = {
  ...
  newsletter: { slug: string; name: string }; publication: { slug: string; name: string }; intro_html?: string; items?: ApiIssueArticle[];
};
...
function mapArticle(item: ApiIssueArticle): Article {
  return {
    ...
    locked: item.locked ?? item.premium, relatedNewsletterSlug: item.newsletter?.slug ?? "", tags: Array.isArray(item.tags) ? item.tags : [],
  };
}
