...
function mapArticle(item: ApiIssueArticle): Article {
  const published = dateValue(item.published_at);
  return {
    slug: item.slug,
    title: item.title,
    dek: item.dek || "",
    ...
    displayDate: dateLabel(item.published_at),
    readTime: "",
...
function mapIssue(item: ApiIssue): NewsletterIssue {
  ...
  return {
    ...
    publishedAt: published.getTime() === 0 ? new Date().toISOString() : published.toISOString(),
    displayDate: dateLabel(item.published_at),
    newsletterSlug: item.newsletter.slug,
    newsletterName: item.newsletter.name,
    publicationSlug: item.publication.slug,
    publicationName: item.publication.name,
    introHtml: item.intro_html || "",
    items: Array.isArray(item.items) ? item.items.map(mapArticle) : undefined,
  };
}
...
export async function getNewsletterIssues(newsletterSlug?: string): Promise<NewsletterIssue[]> {
  ...
  try {
    ...
    console.warn("Hiposta Core issues unavailable.", error);
    return [];
  }
}
...
export async function getNewsletterIssue(slug: string): Promise<NewsletterIssue | null> {
  try {
    ...
    console.warn("Hiposta Core issue detail unavailable.", error);
    return null;
  }
}
