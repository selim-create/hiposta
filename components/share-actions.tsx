...
type Props = {
  ...
  title: string;
  description?: string;
  source: "article" | "publication" | "newsletter" | "issue" | string;
  mode?: "rail" | "inline" | "surface";
  label?: string;
};
...
export function ShareActions({ url, title, description = "", source, mode = "surface", label = "Paylaş" }: Props) {
  const [copied, setCopied] = useState(false);
  const encodedUrl = encodeURIComponent(url);
  const encodedTitle = encodeURIComponent(title);
  const encodedText = encodeURIComponent(`${title} ${url}`);
  const trackShare = (shareChannel: string) => trackAnalyticsEvent({
    eventType: "content_share",
    meta: { source, share_channel: shareChannel },
  });
  ...
  return (
    <div className={`article-share article-share--${mode}`} aria-label={`${title} paylaşım seçenekleri`}>
      <span className="article-share__label">{label}</span>
